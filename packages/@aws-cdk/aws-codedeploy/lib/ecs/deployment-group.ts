 ...
 */
export interface EcsBlueGreenDeploymentOptions {
  /**
  ...
  /**
   * The load balancer listener used to route production traffic to the 'green' ECS task set when a blue-green deployment is complete.
   */
  readonly prodTrafficRoute: elbv2.IListener;
   ...
   */
  readonly testTrafficRoute?: elbv2.IListener;
...
export interface EcsDeploymentGroupProps {
   ...
   */
  readonly blueGreenDeploymentOptions: EcsBlueGreenDeploymentOptions;
...
export class EcsDeploymentGroup extends cdk.Resource implements IEcsDeploymentGroup {
  ...
  constructor(scope: Construct, id: string, props: EcsDeploymentGroupProps) {
    ...
    const resource = new CfnDeploymentGroup(this, 'Resource', {
      ...
      ecsServices: props.services.map<CfnDeploymentGroup.ECSServiceProperty>((service) => {
        return {
      ...
      blueGreenDeploymentConfiguration: cdk.Lazy.any({
        produce: () => this.renderBlueGreenDeploymentConfiguration(props.blueGreenDeploymentOptions),
      }),
      loadBalancerInfo: cdk.Lazy.any({ produce: () => this.renderLoadBalancerInfo(props.blueGreenDeploymentOptions) }),
  ...
  private renderBlueGreenDeploymentConfiguration(options: EcsBlueGreenDeploymentOptions):
  CfnDeploymentGroup.BlueGreenDeploymentConfigurationProperty {
    return {
      deploymentReadyOption: {
        ...
        waitTimeInMinutes: options.deploymentApprovalWaitTime ? options.deploymentApprovalWaitTime?.toMinutes() : 0,
      },
      terminateBlueInstancesOnDeploymentSuccess: {
        action: 'TERMINATE',
        terminationWaitTimeInMinutes: options.terminationWaitTime ? options.terminationWaitTime.toMinutes() : 0,
      },
    };
  }

  private renderLoadBalancerInfo(options: EcsBlueGreenDeploymentOptions): CfnDeploymentGroup.LoadBalancerInfoProperty {
    return {
      targetGroupPairInfoList: [
        {
          ...
          prodTrafficRoute: {
            listenerArns: [
              options.prodTrafficRoute.listenerArn,
            ],
          },
          testTrafficRoute: options.testTrafficRoute ? {
            listenerArns: [
              options.testTrafficRoute.listenerArn,
            ],
          } : undefined,
        },
      ],
